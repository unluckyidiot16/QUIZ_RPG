...
function pickDefaultId(slot, cat) {
...
export async function bootstrapFirstRun() {
    ...
    if (s && s.equipped && Object.keys(s.equipped).length > 0)
        return;
    // 카탈로그 로딩(맵/배열 모두 대응: wearable.catalog 가 normalize 하므로 맵 기준으로 취급)
    const catAny = await loadWearablesCatalog();
    const cat = Array.isArray(catAny)
        ? Object.fromEntries(catAny.filter(Boolean).map((it) => [it.id, it]))
        : catAny;
    ...
    for (const it of Object.values(cat)) {
        ...
        if (it.active === true) {
            if (!activeBySlot.has(slot)) {
                ...
                console.warn("[bootstrap] duplicate active for slot:", slot, "keep=", activeBySlot.get(slot), "ignore=", it.id);
            }
        }
    }
    ...
    // 장착할 항목이 없다면 종료
    if (Object.keys(nextEquip).length === 0)
        return;
    // 3) 적용: 장착 + 보유 추가
    await inv.apply({
        equip: nextEquip,
