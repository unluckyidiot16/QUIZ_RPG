// ---- Env ----
const SHEET_ID = import.meta.env?.VITE_WEAR_SHEET_ID ||
    import.meta.env?.VITE_SHEET_ID ||
    '';
const SHEET_NAME = import.meta.env?.VITE_WEAR_SHEET_NAME ||
    import.meta.env?.VITE_SHEET_NAME ||
    'Wearables';
const ASSETS_ROOT = import.meta.env?.VITE_ASSETS_ROOT?.replace(/\/+$/, '') || '';
// ---- Utils ----
const toNum = (v) => {
    if (v == null || v === '')
        return undefined;
...
const joinUrl = (root, p) => !root ? p.replace(/^\/+/, '') : `${root}/${p.replace(/^\/+/, '')}`;
// gviz 응답에서 JSON만 추출
...
// ---- Main loader ----
export async function loadWearablesCatalog() {
    if (!SHEET_ID)
        return {}; // env 누락 시 깔끔히 폴백
    try {
        ...
        // gviz 표 파싱
        const cols = (j.table?.cols || []).map((c) => String(c?.label ?? '').trim().toLowerCase());
        ...
        for (const r of rows) {
            const c = r.c || [];
            const get = (k) => (k >= 0 ? c[k]?.v ?? c[k]?.f ?? '' : '');
            const id = String(get(I.id) ?? '').trim();
            if (!id)
                continue;
            const active = truthy(get(I.active));
            if (I.active >= 0 && !active)
                continue; // active 컬럼이 있고 false면 스킵
            const name = String(get(I.name) ?? id).trim();
            const slot = String(get(I.slot) ?? '').trim();
