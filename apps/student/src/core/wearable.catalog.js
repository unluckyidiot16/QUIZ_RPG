...
// ===== Config =====
const PACKS_BASE = import.meta.env?.VITE_PACKS_BASE;
...
// ===== Primary loader (JSON) =====
async function fetchJsonCatalog(url, bust = false) {
...
async function fetchSheetCatalog() {
    try {
        ...
        const fn = mod.loadWearablesCatalogFromSheet ||
            mod.loadWearablesCatalog ||
            null;
        if (!fn)
            throw new Error('no sheet loader export');
        const data = await fn();
        return normalizeAny(data);
    }
    catch {
