...
const pickSrc = (it?:any) => norm( it?.src ?? it?.image ?? it?.img ?? it?.renderSrc ?? it?.render ?? it?.thumbnail ?? (Array.isArray(it?.images)?it.images[0]:undefined) ?? (typeof it?.url==='string'?it.url:undefined) );
...
export default function AppHeader(){
  ...
  useEffect(() => {
    ...
    const onChanged = async () => { const s = await inv.load(); setEquipped(s?.equipped ?? {}); };
    window.addEventListener('inv:changed', onChanged as EventListener);
    return () => window.removeEventListener('inv:changed', onChanged as EventListener);
  }, [inv]);
  ...
  const layers = useMemo(() => {
    ...
    for (const slot of SLOTS){
      ...
      if (!src) continue;
      const z = Number.isFinite((it as any)?.layer ?? (it as any)?.z) ? Number((it as any)?.layer ?? (it as any)?.z) : (Z[slot] ?? 0);
