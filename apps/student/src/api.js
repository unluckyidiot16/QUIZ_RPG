...
export const SUPABASE_READY = !!url && /^https?:\/\//i.test(url) && !!anon;
function getClient() {
    if (!SUPABASE_READY)
        throw new Error('[env] Supabase not configured');
    if (!sb)
        sb = createClient(url, anon);
    return sb;
}
// 1) 로그인 → user_id 반환, 로컬 캐시
export async function authLogin(nickname = 'web-student') {
...
export async function finishDungeon(s) {
...
export async function newRunToken() {
